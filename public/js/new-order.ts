    ...
    grid.querySelectorAll('.cat-card').forEach((card) => {
      card.addEventListener('click', (e) => {
        ...
        grid.querySelectorAll('.cat-card').forEach((c) => {
          c.className =
            'cat-card p-3 rounded-xl border cursor-pointer transition-all bg-slate-900 border-slate-700 text-slate-300 hover:border-slate-600';
        });

        target.className =
          'cat-card p-3 rounded-xl border cursor-pointer transition-all bg-brand-600/20 border-brand-500 text-white shadow-md';
      });
    });
  } catch (err: any) {
