...
const checkPassword = (password: string): PasswordValidity => {
...
const updateRuleUI = (element: HTMLElement | null, valid: boolean): void => {
  if (!element) return;
...
// Real-time password feedback listener
if (passwordInput) {
...
if (form && submitBtn) {
  form.addEventListener('submit', async (e: Event) => {
    ...
    // 1. Password Policy Check (8+ chars, at least 1 letter, at least 1 digit)
