...
function renderCustomerSuitCard(order: any): string {
  ...
  return `
    <div class="tailor-card p-5 sm:p-6 rounded-3xl space-y-4 border border-slate-800 shadow-xl">
      <!-- Top Line: Order Number & Status -->
      <div class="flex justify-between items-center flex-wrap gap-2">
        <div class="flex items-center gap-2">
          <span class="font-mono text-lg sm:text-xl font-extrabold text-white">${order.orderNumber}</span>
          <span class="text-xs px-2.5 py-0.5 rounded-full font-bold bg-brand-600/20 text-brand-400 border border-brand-500/30">
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="btn-print-slip px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs border border-slate-700 font-semibold" data-id="${order._id}">
            🖨 Slip
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 border-t border-slate-800 text-xs text-slate-300">
        <div>
        ...
        <div class="sm:text-right">
          <div>Promised Delivery: <strong class="text-white text-sm">${deliveryFormatted}</strong></div>
          <div>Total Price: <strong>${order.totalAmount} PKR</strong> (Advance: ${order.advancePayment || 0} PKR)</div>
        </div>
      </div>
    </div>
  `;
...
function renderMeasurementProfiles(profiles: any[]): void {
  ...
  if (profiles.length === 0) {
    container.innerHTML = `
      <div class="col-span-full p-6 text-center text-slate-500 text-xs bg-slate-900/40 rounded-2xl border border-slate-800">
  ...
  container.innerHTML = profiles
    .map((p: any) => {
      ...
      return `
      <div class="tailor-card p-4 rounded-2xl space-y-3 text-xs border border-slate-800">
        <div class="flex justify-between items-center">
          <strong class="text-white text-sm">${p.title}</strong>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-slate-800 text-brand-400 font-semibold">${p.unit}</span>
        </div>
        
        <div class="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 space-y-2">
          <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Upper / قمیض</div>
          ...
          <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider pt-2 border-t border-slate-800/60">Lower / شلوار</div>
          <div class="grid grid-cols-3 gap-1.5 text-slate-300">
...
function renderCompletedHistory(orders: any[]): void {
  ...
  if (orders.length === 0) {
    container.innerHTML = `
      <div class="p-4 text-center text-slate-500 text-xs bg-slate-900/30 rounded-xl">
  ...
  container.innerHTML = orders
    ...
    <div class="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-between text-xs">
      <div>
        <span class="font-mono font-bold text-white">${o.orderNumber}</span>
        <span class="text-slate-400 ml-2">${o.clothingCategory.toUpperCase()}</span>
        <span class="text-slate-500 ml-2">• Delivered on: ${new Date(o.actualDeliveredDate || o.updatedAt).toLocaleDateString()}</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-emerald-400 font-semibold">${o.totalAmount} PKR</span>
