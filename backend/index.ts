...
// Flexible multi-origin CORS for Admin Panel, Customer Panel, and configured origins
app.use(
  cors({
...
app.use((req: Request, res: Response, next: NextFunction) => {
  if (req.accepts('html') && !req.xhr && !req.headers['x-requested-with']) {
    if (req.path === '/admin' || req.path.startsWith('/admin/')) {
      return res.redirect(`${ADMIN_FRONTEND_URL}${req.path === '/admin' ? '/index.html' : req.path}`);
    }
    if (req.path === '/portal' || req.path.startsWith('/portal/') || req.path === '/customer' || req.path.startsWith('/customer/')) {
      return res.redirect(`${CUSTOMER_FRONTEND_URL}/index.html`);
    }
  }
  next();
});
...
app.get('/', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    services: {
      ...
      customerFrontendUrl: CUSTOMER_FRONTEND_URL,
      restApiBase: '/api',
