/**
 * Action Tailor - Stat Card UI Component
 * TypeScript client-side stat card component
 */
...
const getColorHex = (color: StatCardColor = 'primary'): string => {
  switch (color) {
    case 'success':
      return '#10b981';
    case 'warning':
...
export const createStatCard = (options: StatCardOptions): HTMLElement => {
...
export const updateStatCard = (cardElement: HTMLElement | string, value: string | number, subtitle?: string): void => {
  ...
  if (subtitle !== undefined) {
    let subEl = el.querySelector<HTMLElement>('.stat-subtitle');
    if (!subEl) {
